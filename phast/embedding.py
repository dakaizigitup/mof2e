...
class PhysRef(nn.Module):
    ...
    def __init__(
        ...
        self.short = short
        self.series_mapping = {
            'Nonmetals': 0, 'Noble gases': 1, 'Alkali metals': 2, 'Alkaline earth metals': 3,
            'Metalloids': 4, 'Halogens': 5, 'Poor metals': 6, 'Transition metals': 7,
            'Lanthanides': 8, 'Actinides': 9
        }
        ...
        elements_dict = {z: element(z) for z in df.index}
        # 添加属性
        df['electronegativity_allen'] = [getattr(elements_dict[z], 'en_allen', None) for z in df.index]
        df['nvalence'] = [el.nvalence() if hasattr(el, 'nvalence') else None for z, el in elements_dict.items()]
        df['series'] = [self.series_mapping.get(str(getattr(el, 'series', '')), -1) for el in elements_dict.values()]
        # df['econf'] = [el.econf for el in elements_dict.values()]
        # Add ionization energy
        block_mapping = {'s': 0, 'p': 1, 'd': 2, 'f': 3}
        df['block'] = [block_mapping.get(b, -1) for b in df['block']]
