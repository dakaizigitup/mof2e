...
from fairchem.core.trainers.base_trainer_lora import BaseTrainer_lora

if TYPE_CHECKING:
    from torch_geometric.data import Batch


@registry.register_trainer("ocp_lora")
@registry.register_trainer("energy")
@registry.register_trainer("forces")
class OCPTrainer_lora(BaseTrainer_lora):
    """
    ...
    def train(self, disable_eval_tqdm: bool = False) -> None:
        ...
        # if not hasattr(self,'optimizer') or len(self.optimizer.param_groups) == 1:
        ...
        es_cfg = self.config.get("optim", {}).get("early_stopping", {})
        self._es_enabled = bool(es_cfg.get("enabled", False))
        self._es_patience = int(es_cfg.get("patience", 10))
        self._es_min_delta = float(es_cfg.get("min_delta", 0.0))
        self._es_mode = str(es_cfg.get("mode", "min"))  # min for loss/mae, max for acc
        ...
        # 🎯 新增：在训练开始前进行初始验证（仅当从头开始训练时）
        if start_epoch == 0 and self.val_loader is not None:
            # 保存原始的epoch和step值，验证后恢复
            original_epoch = self.epoch
            original_step = self.step
            
            if distutils.is_master():
                logging.info("=" * 60)
                logging.info("🔍 Initial Model Evaluation (Before Training)")
                logging.info("   Evaluating initial model on validation set...")
                logging.info("=" * 60)
            
            # 临时设置epoch和step为0，用于初始验证日志
            self.epoch = 0.0
            self.step = 0
            ...
            if distutils.is_master():
                logging.info("=" * 60)
                logging.info("📊 Initial Validation Results:")
                for metric_name, metric_data in initial_val_metrics.items():
                    metric_value = metric_data.get("metric", "N/A")
                    logging.info(f"   {metric_name}: {metric_value:.6f}")
                logging.info("=" * 60)
                logging.info("")
            
            # 更新最佳验证指标并保存模型（如果初始效果更好）
            # 使用update_best方法统一处理，它会自动判断是否需要保存
            ...
            if self.best_val_metric != old_best and distutils.is_master():
                logging.info(f"✅ Initial model sets new best {primary_metric}: {self.best_val_metric:.6f}")
                logging.info("   Best checkpoint saved based on initial evaluation.")
        ...
        for epoch_int in range(start_epoch, self.config["optim"]["max_epochs"]):
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                # 获取学习率（多参数组兼容）
                lr_groups = {}
                if hasattr(self, "scheduler") and hasattr(self.scheduler, "scheduler_type"):
                    if self.scheduler.scheduler_type != "Null":
                        try:
                            lr_groups = self.get_lr_dict()      # 你类里已有这个方法
                        except Exception:
                            try:
                                lr_groups = self.scheduler.get_lr_dict()
                            except Exception:
                                lr_groups = {}
                log_dict.update(lr_groups)
                if "lr" not in log_dict and len(lr_groups) == 1:
                    only_key = list(lr_groups.keys())[0]
                    log_dict["lr"] = lr_groups[only_key]

                if (
                    self.step % self.config["cmd"]["print_every"] == 0
                    and distutils.is_master()
                ):
                    log_str = [f"{k}: {v:.2e}" if isinstance(v, (int, float)) else f"{k}: {v}"
                ...
                if self.step % eval_every == 0:
                    if self.val_loader is not None:
                        ...
                        if self._es_enabled:
                            cur = val_metrics[primary_metric]["metric"]
                            best = self.best_val_metric
                            if self._es_mode == "min":
                                improved = cur < (best - self._es_min_delta)
                            else:
                                improved = cur > (best + self._es_min_delta)
                            if improved:
                                self._es_no_improve = 0
                                ...
                                if self._es_no_improve >= self._es_patience:
                                    if distutils.is_master():
                                        logging.info(
                                            f"[EarlyStopping] No improvement in {primary_metric} for {self._es_patience} evals. Stopping."
                                        )
                ...
                # Early Stopping: break inner loop if triggered
                if self._stop_training:
                    break

            torch.cuda.empty_cache()

            # Early Stopping: break outer epoch loop if triggered
            if self._stop_training:
                break
