...
class BaseTrainer(ABC):
    ...
    def load_datasets(self) -> None:
        self.collater = partial(
            data_list_collater,
            otf_graph=self.config["model"].get("otf_graph", False),
            suppress_edge_index_warning=self.config.get("dataset", {}).get(
                "suppress_edge_index_warning", True
            ),
        )
        ...
        if "src" in self.config["dataset"]:
            ...
            logging.info(f"DEBUG: train_dataset length = {len(self.train_dataset)}")
            logging.info(f"DEBUG: train_sampler type = {type(self.train_sampler)}")
            logging.info(f"DEBUG: train_loader type = {type(self.train_loader)}")
            try:
                loader_len = len(self.train_loader)
                logging.info(f"DEBUG: train_loader length = {loader_len}")
            except Exception as e:
                logging.error(f"DEBUG: Failed to get train_loader length: {e}")
    ...
    def load_model(self) -> None:
        ...
        if distutils.initialized():
            find_unused_parameters = self.config.get("task", {}).get(
        ...
        if inference_only is False:
            ...
            if "optimizer" in checkpoint:
                try:
                    ...
                    logging.warning(f"Skipping optimizer loading due to mismatch (safe for validation/inference): {e}")
    ...
    def load_optimizer(self) -> None:
        ...
        # Check if layered learning rate is enabled (for multi-stream model)
        backbone_lr_ratio = self.config["optim"].get("backbone_lr_ratio", None)
        base_lr = self.config["optim"]["lr_initial"]
        ...
        if weight_decay > 0:
            ...
            # Separate parameters by: backbone vs new modules, and weight decay
            backbone_decay, backbone_no_decay = [], []
            new_modules_decay, new_modules_no_decay = [], []
            name_no_decay = []
            ...
            for name, param in self.model.named_parameters():
                ...
                # Check if parameter belongs to backbone (GNN)
                is_backbone = (
                    'gnn_mof' in name or 
                    'escn' in name or
                    'backbone' in name
                ...
                no_wd = any(name.endswith(skip_name) for skip_name in self.model_params_no_wd)
                
                if is_backbone:
                    if no_wd:
                        backbone_no_decay.append(param)
                        if backbone_lr_ratio is not None:
                            name_no_decay.append(f"{name} (backbone, lr={base_lr * backbone_lr_ratio})")
                    else:
                        backbone_decay.append(param)
                else:
                    if no_wd:
                        new_modules_no_decay.append(param)
                        name_no_decay.append(name)
                    else:
                        new_modules_decay.append(param)
            ...
            if backbone_lr_ratio is not None:
                # Layered LR: backbone uses smaller learning rate
                backbone_lr = base_lr * backbone_lr_ratio
                logging.info(f"Using layered learning rates: backbone_lr={backbone_lr:.6f}, new_modules_lr={base_lr:.6f}")
                
                if backbone_no_decay:
                    param_groups.append({"params": backbone_no_decay, "weight_decay": 0, "lr": backbone_lr})
                if backbone_decay:
                    param_groups.append({"params": backbone_decay, "weight_decay": weight_decay, "lr": backbone_lr})
                if new_modules_no_decay:
                    param_groups.append({"params": new_modules_no_decay, "weight_decay": 0, "lr": base_lr})
                if new_modules_decay:
                    param_groups.append({"params": new_modules_decay, "weight_decay": weight_decay, "lr": base_lr})
            else:
                # Standard: all parameters use same learning rate
                params_no_decay = backbone_no_decay + new_modules_no_decay
                params_decay = backbone_decay + new_modules_decay
                
                if params_no_decay:
                    param_groups.append({"params": params_no_decay, "weight_decay": 0})
                if params_decay:
                    param_groups.append({"params": params_decay, "weight_decay": weight_decay})

            self.optimizer = optimizer(
                params=param_groups,
                lr=base_lr if backbone_lr_ratio is None else base_lr,  # Base LR for param groups without explicit lr
                **optimizer_params,
            )
        else:
            self.optimizer = optimizer(
    ...
    def validate(self, split: str = "val", disable_tqdm: bool = False):
        ...
        # ---- 验证阶段：全局特征覆盖率统计（只在 master 上打印一次）----
        # 如果模型使用了 MOFGlobalPropertyEncoder，则在一次 validate 内统计 name 命中/缺失情况
        if distutils.is_master():
            try:
                m = self._unwrapped_model
                backbone = getattr(m, "backbone", None)
                enc = getattr(backbone, "mof_global_encoder", None) if backbone is not None else None
                if enc is not None and hasattr(enc, "reset_coverage_stats"):
                    enc.reset_coverage_stats()
            except Exception:
                pass
        ...
        if distutils.is_master():
            try:
                ...
                backbone = getattr(m, "backbone", None)
                enc = getattr(backbone, "mof_global_encoder", None) if backbone is not None else None
                if enc is not None and hasattr(enc, "report_coverage_stats"):
                    enc.report_coverage_stats(prefix=f"{split}_global_feature")
            except Exception:
                pass
