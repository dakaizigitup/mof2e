...
class SO3_Embedding(torch.nn.Module):
    ...
    def _grid_act(self, SO3_grid, act, mappingReduced) -> None:
        offset = 0
        # Chunk the edge-expanded tensor along the first dimension to avoid
        # materializing the full z-b-a-c grid activation at once. This keeps
        # the numerics unchanged while substantially lowering the peak memory
        # in eSCN message passing, especially during checkpoint recomputation.
        chunk_target_mb = int(os.environ.get("FAIRCHEM_ESCN_GRID_CHUNK_MB", "256"))
        for i in range(self.num_resolutions):
            num_coefficients = mappingReduced.res_size[i]
            ...
            # x_grid has shape [z, b, a, c]. Choose a chunk size so a single
            # activation block stays near chunk_target_mb.
            n_entries = x_res.shape[0]
            elem_size = max(1, x_res.element_size())
            grid_slice_elems = (
                max(1, to_grid_mat.shape[0])
                * max(1, to_grid_mat.shape[1])
                * max(1, x_res.shape[2])
            )
            max_chunk = max(
                1,
                (chunk_target_mb * 1024 * 1024) // (grid_slice_elems * elem_size),
            )

            if n_entries <= max_chunk:
                x_grid = torch.einsum("bai,zic->zbac", to_grid_mat, x_res)
                x_grid = act(x_grid)
                x_res = torch.einsum("bai,zbac->zic", from_grid_mat, x_grid)
            else:
                x_res_out = torch.empty_like(x_res)
                for start in range(0, n_entries, max_chunk):
                    end = min(start + max_chunk, n_entries)
                    x_grid = torch.einsum(
                        "bai,zic->zbac", to_grid_mat, x_res[start:end]
                    )
                    x_grid = act(x_grid)
                    x_res_out[start:end] = torch.einsum(
                        "bai,zbac->zic", from_grid_mat, x_grid
                    )
                x_res = x_res_out
